...
/**
 * InsightReactiveMediumSeverityHandler for requests to Lambda function.
 */
public class InsightReactiveMediumSeverityHandler implements RequestHandler<Map<String, Object>, String> {
    ...
    public String handleRequest(Map<String, Object> input, final Context context) {
        ...
        // Filter only Reactive and Medium severity insights
        if (!InsightSeverity.MEDIUM.toString().equalsIgnoreCase(jsonNodeDetail.path("insightSeverity").asText())
                || !InsightType.REACTIVE.toString().equalsIgnoreCase(jsonNodeDetail.path("insightType").asText())) {
